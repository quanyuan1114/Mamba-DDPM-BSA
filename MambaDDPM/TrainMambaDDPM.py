...
from Mamba.mambaUNet import MambaUNet
...
def sample(cfg):
    ...
    transform = transforms.Compose([
        transforms.ToPILImage(),  # 将生成图片张量转换为 PIL 图像
        transforms.Resize(224),  # 调整图像尺寸为模型所需的大小（224x224）
        transforms.ToTensor(),  # 将 PIL 图像转换为张量，并归一化至 [0, 1]
